...
void computeLSFit(lsFit *fitDat, matchResult *matches, lsTiepoints *tiePoints)
{
	...
	if(fitDat->fitType==CONSTANTFIT) { 
		ma=1; 
		fprintf(stderr,"** Fitting for a constant ****\n");
	}
	else if(fitDat->fitType==PLANEFIT) { 
		ma=3; fprintf(stderr,"** Fitting for a plane ****\n");
	...
	while(notDone == TRUE && nIterations < MAXTIEITERATIONS) {
		if(fitDat->fitType == PLANEFIT) {
			...
			fprintf(stderr,"CX_%1i1_%1i2_%1i3 %9.6le %9.6le %9.6le \n",1, 1,1,
				CmatrixX[1][1], CmatrixX[1][2]*MTOKM, CmatrixX[1][3]*MTOKM);
       		fprintf(stderr,"CX_%1i1_%1i2_%1i3 %9.6le %9.6le %9.6le \n",2,2,2, 
				CmatrixX[2][1]*MTOKM, CmatrixX[2][2]*MTOKM*MTOKM,CmatrixX[2][3]*MTOKM*MTOKM);
       		fprintf(stderr,"CX_%1i1_%1i2_%1i3 %9.6le %9.6le %9.6le \n",3,3,3,
				CmatrixX[3][1]*MTOKM, CmatrixX[3][2]*MTOKM*MTOKM,CmatrixX[3][3]*MTOKM*MTOKM);
			fprintf(stderr,"CY_%1i1_%1i2_%1i3 %9.6le %9.6le %9.6le \n",1,1,1,
				CmatrixY[1][1], CmatrixY[1][2]*MTOKM, CmatrixY[1][3]*MTOKM);
       		fprintf(stderr,"CY_%1i1_%1i2_%1i3 %9.6le %9.6le %9.6le \n",2,2,2,
				CmatrixY[2][1]*MTOKM, CmatrixY[2][2]*MTOKM*MTOKM,CmatrixY[2][3]*MTOKM*MTOKM);
       		fprintf(stderr,"CY_%1i1_%1i2_%1i3 %9.6le %9.6le %9.6le \n",3,3,3,
				CmatrixY[3][1]*MTOKM, CmatrixY[3][2]*MTOKM*MTOKM,CmatrixY[3][3]*MTOKM*MTOKM);
