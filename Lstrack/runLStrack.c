...
static int32_t getLatePosition(LSimage *earlyImage, LSimage *lateImage, matchParams *matchP,
							   uint32_t ie, uint32_t je, uint32_t *il, uint32_t *jl, int32_t *chipSize,
							   int32_t *lx1, int32_t *lx2, int32_t *ly1, int32_t *ly2);
static float fastCorr(LSimage *earlyImage, LSimage *lateImage, int32_t chipSize, int32_t ie, int32_t je, int32_t il,
					  int32_t jl, int32_t lx1, int32_t lx2, int32_t ly1, int32_t ly2, float *offX, float *offY);
static unsigned char isMasked(double x, double y, matchParams *matchP);
static uint8_t velGuess(double x, double y, matchParams *matchP, double dx, double dy, int32_t *iVel, int32_t *jVel,
						int32_t *slow, int32_t *highStrain);
static float **lsmatrix(int32_t nrl, int32_t nrh, int32_t ncl, int32_t nch);
static float overSamplePeakCorr(int32_t iMax, int32_t jMax, float *offX, float *offY);
...
matchResult runLStrack(char *earlyFile, char *lateFile, matchParams *matchP)
{
	...
	uint32_t idum, idum1;								/* Dummy variables */
	void *pdum;											/* Dummy variables */
	int32_t im, jm, ie, je, il, jl, i, j;				/* LCV - counters for main loops */
	...
	int32_t slow, highStrain;							/* Set when velocity map indicates chip can be increased and search region reduced */
	int32_t chipSize;									/* Chipsize */
	int32_t lx1, ly1, lx2, ly2, k, k1, iVel, jVel;		/* Search region  */
	float offX, offY;									/* Returned offsets - i.e, final result */
	uint32_t edgeBuf;									/* "margin" around edges - i.e., don't use data within edgebuf of image edges */
	...
	*/
	...
	countF = 0;
	matchP->deltaX = 100;
	matchP->deltaY = 100; /* In pixels */
	matchP->nx = 0;
	matchP->ny = 0;
	nMatch = 0;
	nAttempt = 0;
	nTotal = 0; /* Zero counters */
	edgeBuf = 100;
	/*
	...
	*/
	...
	tifEarly = XTIFFOpen(earlyFile, "r");
	if (!tifEarly)
		error("couldn't open tifEarly");
	tifLate = XTIFFOpen(lateFile, "r");
	if (!tifLate)
		error("couldn't open tifLate");
	gtifEarly = GTIFNew(tifEarly);
	if (!gtifEarly)
		error("couldn't open geo tif Early");
	gtifLate = GTIFNew(tifLate);
	if (!gtifLate)
		error("couldn't open geo tif Late");
	/*
	  Read in images
	*/
	loadTiff(earlyFile, &earlyImage, tifEarly, gtifEarly);
	matches.jdEarly = earlyImage.jd;
	loadTiff(lateFile, &lateImage, tifLate, gtifLate);
	matches.jdLate = lateImage.jd;
	...
	*/
	...
	else
	{
		...
		if (earlyImage.proj == NSIDCSOUTH || earlyImage.proj == NSIDCNORTH)
			matchP->proj = earlyImage.proj;
		else if (lateImage.proj == NSIDCSOUTH || lateImage.proj == NSIDCNORTH)
			matchP->proj = lateImage.proj;
	}
	if (matchP->proj != NSIDCSOUTH && matchP->proj != NSIDCNORTH && matchP->proj != NULLPROJ)
		error("Invalid Projection either not set in tiff or cmd line %i \n", matchP->proj);
	...
	*/
	minX = max(earlyImage.x0, lateImage.x0);
	minY = max(earlyImage.y0, lateImage.y0);
	/* Ensure origin for stepping is integer number of pixels in first image */
	if (minX != earlyImage.x0)
	{
		idum = (uint32_t)((minX - earlyImage.x0) / earlyImage.dx);
		minX = earlyImage.x0 + idum * earlyImage.dx;
	}
	if (minY != earlyImage.y0)
	{
		idum = (uint32_t)((minY - earlyImage.y0) / earlyImage.dy);
		minY = earlyImage.y0 + idum * earlyImage.dy;
	}
	matches.x0 = minX + matchP->deltaX * earlyImage.dx;
	matches.y0 = minY + matchP->deltaY * earlyImage.dy;
	matches.dx = earlyImage.dx;
	matches.dy = earlyImage.dy;
	matches.stepX = matchP->stepX;
	matches.stepY = matchP->stepY;
	if (fabs(earlyImage.dx - lateImage.dx) > 0.0001 || fabs(earlyImage.dy - lateImage.dy) > 0.0001)
	{
		error("Image pixel sizes differ %10.5f %10.5f %10.5f %10.5f\n", earlyImage.dx, earlyImage.dy,
	...
	*/
	nX = (uint32_t)((maxX - matches.x0) / earlyImage.dx) - edgeBuf;
	nX = nX / matchP->stepX; /*  pixels, then steps  */
	nY = (uint32_t)((maxY - matches.y0) / earlyImage.dy) - edgeBuf;
	nY = nY / matchP->stepY;
	fprintf(stderr, "nX,nY %lf %lf  nx,ny %i %i	\n", nX, nY, matchP->nx, matchP->ny);

	if ((matchP->nx) <= 0)
	{
		matches.nx = (uint32_t)nX;
	}
	else
	{
...
*************************************** ****************************************/
static uint8_t velGuess(double x, double y, matchParams *matchP, double dx, double dy, int32_t *iVel, int32_t *jVel, int32_t *slow, int32_t *highStrain)
{
	int32_t im, jm, i, j;
	...
	*jVel = 0;
	/*
	  no velocity
	*/
	if (matchP->velFile == NULL)
		return (FALSE); /* no velocity, so return */
	...
	if (jm < 0 || im < 0 || jm >= matchP->velocity.nx || im >= matchP->velocity.ny)
		return (FALSE); /* outside of bounds -> no velocity */

	if (matchP->velocity.vx[im][jm] < (NODATA + 1) || matchP->velocity.vy[im][jm] < (NODATA + 1))
		return (FALSE); /* no velocity value */
	...
	/*
	   Interpolate vx
	*/
	p1 = matchP->velocity.vx[im][jm];
	p2 = matchP->velocity.vx[im][jm + 1];
	p3 = matchP->velocity.vx[im + 1][jm + 1];
	p4 = matchP->velocity.vx[im + 1][jm];
	if (p1 <= (NODATA + 1) || p2 <= (NODATA + 1) || p3 <= (NODATA + 1) || p4 <= (NODATA + 1))
		vxPt = max(max(max(p1, p2), p3), p4);
	else
		vxPt = (float)((1.0 - t) * (1.0 - u) * p1 + t * (1.0 - u) * p2 + t * u * p3 + (1.0 - t) * u * p4);
	/*	fprintf(stderr,"%lf %f %i %i\ %lf %lf \n",(double)(vxPt/365.), matchP->velocity.vx[im][jm],im,jm,t,u);*/
	/* projected delta in pixels */
	*jVel = (int32_t)((vxPt / 365.) * matchP->deltaT / dx);
	/*
	   Interpolate vy
	*/
	p1 = matchP->velocity.vy[im][jm];
	p2 = matchP->velocity.vy[im][jm + 1];
	p3 = matchP->velocity.vy[im + 1][jm + 1];
	p4 = matchP->velocity.vy[im + 1][jm];
	if (p1 <= (NODATA + 1) || p2 <= (NODATA + 1) || p3 <= (NODATA + 1) || p4 <= (NODATA + 1))
		vyPt = max(max(max(p1, p2), p3), p4);
	else
		vyPt = (float)((1.0 - t) * (1.0 - u) * p1 + t * (1.0 - u) * p2 + t * u * p3 + (1.0 - t) * u * p4);
	/* projected delta in pixels */
	*iVel = (int32_t)((vyPt / 365.) * matchP->deltaT / dy);
	/*
	...
	*/
	...
	if (matchP->slowFlag == TRUE)
	{
		for (i = im - n; i <= im + n; i++)
		{
			for (j = jm - n; j <= jm + n; j++)
			{
				if (matchP->velocity.vx[i][j] < (NODATA + 1) || matchP->velocity.vy[i][j] < (NODATA + 1))
					return (TRUE); /* Don't bother to near a hole */
		...
		if (sigma < SIGMATHRESH && mean < SLOWTHRESH)
			*slow = 1;
		else if (sigma < 4 * SIGMATHRESH && mean < MIDTHRESH)
			*slow = 2;
...
static unsigned char isMasked(double x, double y, matchParams *matchP)
{
	...
	/*
	  no mask
	*/
	if (matchP->maskFile == NULL)
		return (FALSE); /* no mask, so not masked */

	jm = (int32_t)((x - matchP->mask.x0) / matchP->mask.dx + 0.5);
	im = (int32_t)((y - matchP->mask.y0) / matchP->mask.dy + 0.5);
	if (jm < 0 || im < 0 || jm >= matchP->mask.nx || im >= matchP->mask.ny)
		return (FALSE); /* outside of bounds -> no masked */

	if (matchP->mask.m[im][jm] > 0)
		return (FALSE); /* Positive mask value indicates not masked */
...
*************************************** ****************************************/
static float fastCorr(LSimage *earlyImage, LSimage *lateImage, int32_t chipSize, int32_t ie, int32_t je,
					  int32_t il, int32_t jl, int32_t lx1, int32_t lx2, int32_t ly1, int32_t ly2, float *offX, float *offY)
{
	int32_t i, j; /* Outer loop to move search window */
	int32_t ii, jj, iiMin, iiMax, jjMin, jjMax;
	double meanL, meanE, varE, varL, Xcorr, sigmaE2, sigmaL2, sigmaMaxE, sigmaMaxL;
	int32_t lx1a, lx2a, ly1a, ly2a;
	...
	*/
	if ((il + iiMax) > lateImage->ny || (ie + iiMax) > earlyImage->ny || (jl + jjMax) > lateImage->nx || (je + jjMax) > earlyImage->nx)
	{
		return (NOUSEABLEDATA);
	}
	if ((il + iiMin) < 0 || (ie + iiMin) < 0 || (jl + jjMin) < 0 || (je + jjMin) < 0)
	{
		return (NOUSEABLEDATA);
	}
	/*
	...
	*/
	...
	for (ii = iiMin; ii <= iiMax; ii++)
	{
		...
		{
			if (lateImage->fimage[il + ii][jl + jj] < (NODATA + 1))
			{
				return (NOUSEABLEDATA);
			}
	...
	varL = varL / npts;
	meanL = meanL / npts;
	sigmaL2 = varL - meanL * meanL; /* Normalize variables */
	/*
	...
	*/
	for (ii = iiMin + ly1; ii <= iiMax + ly2; ii++)
	{
		...
		{
			if (earlyImage->fimage[ie + ii][je + jj] < (NODATA + 1))
			{
				return (NOUSEABLEDATA);
			}
		}
	...
	for (i = ly1; i <= ly2; i++)
	{
		for (j = lx1; j <= lx2; j++)
		{
			...
			varE = varE / npts;
			meanE = meanE / npts;
			sigmaE2 = varE - meanE * meanE; /* Normalize variables */
			Xcorr = Xcorr / npts;
	...
	*/
	...
	/*-----------------------------------------------------------------------------------------------------------
	  ...
	  -----------------------------------------------------------------------------------------------------------*/
	if (jMax > lx2 || jMax<lx1 | iMax> ly2 || iMax < ly1)
	{
		*offX = NODATA;
		*offY = NODATA;
		return (NOCORR);
	...
	lx2a = lx1 - 1;
	ly1a = ly2 + 1;
	ly2a = ly1 - 1; /* Will cause to skip loop unless modified below */
	if ((lx2 - jMax) <= OSSIZE)
		lx2a = jMax + OSSIZE;
	if ((ly2 - iMax) <= OSSIZE)
		ly2a = iMax + OSSIZE;
	...
	*/
	for (i = ly1a; i <= ly2a; i++)
	{
		for (j = jMax - OSSIZE; j <= lx2a + OSSIZE; j++)
		{
			...
			varE = 0.0;
			meanE = 0.0;
			sigmaE2 = 0.0; /* Zero summation variables */
			/* Inner correlation loop */
			for (ii = iiMin; ii <= iiMax; ii++)
			{
				for (jj = jjMin; jj <= jjMax; jj++)
	...
	*/
	...
	{ /* technically this will excute empty loop if not needed */
		for (j = lx1a; j <= lx2a; j++)
		{
			Xcorr = 0.0;
			varE = 0.0;
...
static float overSamplePeakCorr(int32_t iMax, int32_t jMax, float *offX, float *offY)
{
	...
	extern fftwf_complex *corrPatchSpace, *corrPatchFreq, *corrPatchOverFreq, *corrPatchOverSpace;
	extern fftwf_plan forwardCorrPlan, inverseCorrPlan;
	int32_t i, j, i1, j1, i2, j2, width, widthO, iMax1, jMax1;
	float maxCorrHi;
	float tmpCorr;
	float Crms;
	width = 2 * OSSIZE;
...
{
	uint16_t *buf, *tileBuf, **tile, **Im, *stripBuf; /* Buffers to read in image */
	float *fbuf;
	uint32_t i, j, i1, j1;			 /* LCV */
	size_t bufSize;					 /* Used to do buffer size math */
	uint32_t tileWidth, tileLength;	 /* Tile size info */
	uint32_t x, y;					 /* Used to index into tiles */
	int32_t ik, jk, nk;				 /* loop indices, for filter convolution, nk=filter halfwidth width=2*nk+1 */
	uint32_t idum;					 /* Dummy variable */
	...
	uint32_t *bc, *sc;
	int32_t rowsperstrip, stripoffsets, stripbytecounts, strip, idum1;
	/*
	...
	*/
	...
	/*
	   Tiff data
	*/
	TIFFGetField(tif, TIFFTAG_IMAGEWIDTH, &idum);
	Image->nx = idum;
	TIFFGetField(tif, TIFFTAG_IMAGELENGTH, &idum);
	Image->ny = idum;
	...
	*/
	...
	yc = 0.0;
	xc1 = 1;
	yc1 = 1; /* Coordinates from origin and one pixel from origin */
	GTIFImageToPCS(gtif, &xc, &yc);
	...
	Image->proj = idum1;
	if (Image->proj != NSIDCNORTH && Image->proj != NSIDCSOUTH)
		Image->proj = -1;
	/*
	   Malloc image buffer
	*/
	bufSize = Image->ny * Image->nx * sizeof(uint16_t);
	buf = malloc(bufSize);
	bufSize = Image->ny * sizeof(uint16_t *);
	Image->image = (uint16_t **)malloc(bufSize);
	for (i = 0; i < Image->ny; i++)
		Image->image[i] = &(buf[i * Image->nx]);
	/*
	...
	*/
	...
	for (i = 0; i < Image->ny; i++)
		Image->fimage[i] = &(fbuf[i * Image->nx]);
	/*
	  Strip info
	*/
	...
	{ /* Tiled */
		...
		*/
		...
		bufSize = tileLength * sizeof(uint16_t *);
		tile = (uint16_t **)malloc(bufSize);
		for (i = 0; i < tileLength; i++)
			tile[i] = &(tileBuf[i * tileWidth]);
		/*
		  read tiles
		*/
	...
	*/
	...
	for (i = 0; i < Image->ny; i++)
		...
		{
			Image->fimage[i][j] = (float)NODATA;
			Image->fimage[i][Image->nx - j - 1] = (float)NODATA;
		}
	for (j = 0; j < Image->nx; j++)
	{
		...
		{
			Image->fimage[i][j] = (float)NODATA;
			Image->fimage[Image->ny - i - 1][j] = (float)NODATA;
		}
	}
	free(Image->image[0]);
	/*
...
static void parseLSDate(char *file, LSimage *Image)
{
	int32_t i, j, slash, idum, leadZero, path, row, year, doy, jd;
	char tmp[1500], *tmp1;
	i = 0;
	slash = 0;
	while (file[i] != '\0')
	{
		...
		i++;
		if (i > 1500)
			error("Error parsing landsat file %s\n", file);
	}
	tmp1 = &(file[slash + 1]);
	fprintf(stderr, "%s %s \n", file, tmp1);
	fprintf(stderr, "%i \n", slash);
	Image->sat = (char *)malloc(4 * sizeof(char));
	for (i = 0; i < 3; i++)
		Image->sat[i] = tmp1[i];
	Image->sat[3] = '\0';
	if (tmp1[2] == '0')
...
static void mallocMatch(matchResult *matches)
{
	...
	int8_t *bufu;	/* Pointer to malloced memory for correlation buffer */
	size_t bufSize; /* Used to do buffer size math */
	int32_t width, widthO;
	uint32_t i, j; /* LCV */
	extern float **corr;
	...
	*/
	...
	for (i = 0; i < widthO; i++)
	{
		for (j = 0; j < widthO; j++)
		{
			corrPatchOverFreq[i * widthO + j][0] = 0.0;
			corrPatchOverFreq[i * widthO + j][1] = 0.0;
		}
	}
	...
	*/
	...
	bufSize = sizeof(uint8_t *) * matches->ny;
	matches->type = (uint8_t **)malloc(bufSize);
	...
	for (i = 0; i < matches->ny; i++)
		matches->Rho[i] = &(buf[i * matches->nx]);

	buf = (float *)malloc(bufSize);
	for (i = 0; i < matches->ny; i++)
		matches->X[i] = &(buf[i * matches->nx]);

	buf = (float *)malloc(bufSize);
	for (i = 0; i < matches->ny; i++)
		matches->Y[i] = &(buf[i * matches->nx]);

	bufSize = sizeof(int8_t) * matches->ny * matches->nx;
	fprintf(stderr, "bufSize %li\n", bufSize);
	bufu = (int8_t *)malloc(bufSize);
	for (i = 0; i < matches->ny; i++)
		matches->type[i] = &(bufu[i * matches->nx]);
...
*/
float **lsmatrix(int32_t nrl, int32_t nrh, int32_t ncl, int32_t nch)
...
static void writeTiffAsRaw(LSimage *Image, char *file)
{
	...
	FILE *fp;
	fp = fopen(file, "w");
	/*      fwriteBS(Image->image[0],sizeof(uint16_t),(size_t)(Image->nx*Image->ny),fp,INT16FLAG); */
