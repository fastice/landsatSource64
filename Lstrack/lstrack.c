...
{   
	...
	*/
	...
	else fprintf(stderr,"maskFile = NULL\n");
    if(matchP.velFile != NULL) fprintf(stderr,"velFile = %s\n", matchP.velFile);
	else fprintf(stderr,"velFile = NULL\n");
...
static void writeLSOffsets(matchResult *matches, matchParams *matchP)
{
	...
	for(i=0; i<sl; i++) file1[i]='\0'; 
	file1=strcpy(file1,matchP->outputFile); 
	file1=strcat(file1,".dy"); fprintf(stderr,"%s\n",file1);
	fp=fopen(file1,"w");  
	...
	for(i=0; i<sl; i++) file1[i]='\0'; 
	file1=strcpy(file1,matchP->outputFile); 
	file1=strcat(file1,".dat"); fprintf(stderr,"%s\n",file1);
	fp=fopen(file1,"w"); 
	...
	if(nAttempt > 0) successRate = ( (float)nMatch/(float)nAttempt)*100.; 
