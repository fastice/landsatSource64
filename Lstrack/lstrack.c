...
int main(int argc, char *argv[])
{
	...
	*/
	...
	if (matchP.maskFile != NULL)
		fprintf(stderr, "maskFile = %s\n", matchP.maskFile);
	else
		fprintf(stderr, "maskFile = NULL\n");
	if (matchP.velFile != NULL)
		fprintf(stderr, "velFile = %s\n", matchP.velFile);
	else
		fprintf(stderr, "velFile = NULL\n");
	...
	*/
	if (matchP.maskFile != NULL)
		readLSMaskFile(&matchP);
	/*
	  Read vel file
	*/
	if (matchP.velFile != NULL)
		readVelMosaic(&matchP);
	/*
	  Call matcher
	*/
	fprintf(stderr, "Run tracker...\n");
...
static void writeLSOffsets(matchResult *matches, matchParams *matchP)
{
	...
	file1 = strcat(file1, ".rho");
	fprintf(stderr, "%s\n", file1);

	fp = fopen(file1, "w");
	fwriteBS(matches->Rho[0], sizeof(float), (size_t)(matches->nx * matches->ny), fp, FLOAT32FLAG);
	fclose(fp);
	...
	for (i = 0; i < sl; i++)
		file1[i] = '\0';
	file1 = strcpy(file1, matchP->outputFile);
	file1 = strcat(file1, ".dx");
	fprintf(stderr, "%s\n", file1);
	fp = fopen(file1, "w");
	fwriteBS(matches->X[0], sizeof(float), (size_t)(matches->nx * matches->ny), fp, FLOAT32FLAG);
	fclose(fp);

	for (i = 0; i < sl; i++)
		file1[i] = '\0';
	file1 = strcpy(file1, matchP->outputFile);
	file1 = strcat(file1, ".dy");
	fprintf(stderr, "%s\n", file1);
	fp = fopen(file1, "w");
	fwriteBS(matches->Y[0], sizeof(float), (size_t)(matches->nx * matches->ny), fp, FLOAT32FLAG);
	fclose(fp);

	for (i = 0; i < sl; i++)
		file1[i] = '\0';
	file1 = strcpy(file1, matchP->outputFile);
	file1 = strcat(file1, ".mtype");
	fprintf(stderr, "%s\n", file1);
	fp = fopen(file1, "w");
	...
	if (nAttempt > 0)
		successRate = ((float)nMatch / (float)nAttempt) * 100.;
	else
		successRate = 0.0;
...
static void readVelMosaic(matchParams *matchP)
{
	...
	/*
	  geodat file name
	*/
	geodatFile = (char *)malloc(strlen(matchP->velFile) + 11);
	geodatFile[0] = '\0';
	...
	/*
	   velocity file names
	*/
	vxFile = (char *)malloc(strlen(matchP->velFile) + 4);
	vxFile[0] = '\0';
	vxFile = strcpy(vxFile, matchP->velFile);
	vxFile = strcat(vxFile, ".vx");

	vyFile = (char *)malloc(strlen(matchP->velFile) + 4);
	vyFile[0] = '\0';
	vyFile = strcpy(vyFile, matchP->velFile);
	...
	*/
	...
	matchP->velocity.x0 *= KMTOMS;
	matchP->velocity.y0 *= KMTOMS;
	fclose(fp);
...
static void readLSMaskFile(matchParams *matchP)
{
	...
	*/
	...
	matchP->mask.x0 *= KMTOMS;
	matchP->mask.y0 *= KMTOMS;
	fclose(fp);
...
static void readArgs(int argc, char *argv[], char **earlyFile, char **lateFile, matchParams *matchP)
{
	...
	matchP->chipSize = 33;
	matchP->searchX = MINSEARCH;
	matchP->searchY = MINSEARCH;
	matchP->stepX = DEFAULTSTEP;
	matchP->stepY = DEFAULTSTEP;
	...
	*earlyFile = argv[argc - 3];
	...
	for (i = 1; i <= n; i++)
	{
		...
		{
			sscanf(argv[i + 1], "%i", &idum);
			if (idum != NSIDCNORTH && idum != NSIDCSOUTH && idum != NULLPROJ)
				error("invalid projection, choices are 3413 and 3031 or 9999 (null)\n");
			matchP->proj = idum;
			i++;
		}
